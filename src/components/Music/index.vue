<template>
  <div
    ...
    ref="music"
    @mouseover="showMusic"
    @mouseleave="WatchMusic"
  >
    <audio :src="music" ref="audio"></audio>
    <div
      class="player-wrapper notChoose"
      @mouseup="stopChange()"
      @mousemove="getX($event)"
    >
        ...
        <div class="bar">
          <div class="barbg" @mousedown="changePosition($event)">
            ...
            <div class="cur" :style="'width:' + `${width}` + '%'">
              <div @click.stop>
                <div class="btn" @mousedown="goChange()"></div>
              </div>
            </div>
          </div>
          <div class="time">
      ...
      <div class="list" :style="'top:' + `${list ? 0 + '%' : 100 + '%'}`">
        ...
        <div class="listbd">
          ...
          <div class="listbdc" v-if="musiclist.length != 0">
            <ul class="clearfix">
              <li
                ...
                :key="index"
                @click="setMusic(song.id)"
                :class="{ musicActive: song.id == musicNow.id }"
              >
                <div class="col col1">
                ...
                <div class="col col4">
                  <span v-for="(artist, index) in song.ar" :key="index">
                    <router-link
                      :to="{ path: '/artist/song', query: { id: artist.id } }"
                      :title="artist.name"
                      class="underline art"
          ...
          <div class="nocnt" v-else>
            ...
            <br />
            <span
              >去首页<router-link to="/home/discover">发现音乐</router-link
              >，或在<router-link to="/mymusic">我的音乐</router-link
              >收听自己收藏的歌单</span
            >
          </div>
          <div class="bline"></div>
  ...
  methods: {
    ...
    pre() {
      ...
      if (this.loop == 1 || this.loop == 3) {
        ...
        this.setMusic(
          this.musiclist[parseInt(Math.random() * this.musiclist.length)].id
        );
      }
    },
    // 下一首歌
    ...
    WatchMusic() {
      this.timer = setInterval(() => {
        ...
        if (!this.locked && !this.list) this.$refs.music.style.height = "8px";
        clearInterval(this.timer);
      }, 1000);
    },
    // 进度条监视
    WatchBar() {
      ...
      this.timer2 = setInterval(() => {
        ...
        this.duration =
          this.min(this.audio.duration * 1000) +
          ":" +
          this.sec(this.audio.duration * 1000);
        this.nowTime =
          this.min(this.audio.currentTime * 1000) +
          ":" +
          this.sec(this.audio.currentTime * 1000);
        // 播放完毕
    ...
    // 调整音量大小
    changeVolume(e) {
      this.volumeHeight = Math.max(
        Math.min(((693 - e.clientY) / 87) * 100 - 10, 80),
        0
      );
      this.audio.volume = this.volumeHeight / 80;
    },
    ...
    getY(e) {
      if (this.isVolume == true) {
        ...
        this.audio.volume = this.volumeHeight / 100;
      }
    },
    //停止拖拽
    stopVolume() {
      if (this.isVolume == true) this.isVolume = false;
    },
    // 显示和隐藏歌曲列表
    showList() {
    ...
    min(song) {
      if (parseInt(song / 60000) < 10) return "0" + parseInt(song / 60000);
